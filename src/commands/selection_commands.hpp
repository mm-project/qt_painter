...
class incmdSelectUnderCursoer : public InteractiveCommandBase {
  ...
  public:
    incmdSelectUnderCursoer(RuntimePoolManagerPtr r, ObjectPoolPtr s)
        : m_re(r), m_ws(s) {
    ...
    virtual std::string get_name() { return "incmdSelectUnderCursoer"; }

    virtual void execute() {
        InteractiveCommandBase::set_next_handler(
            HANDLE_FUNCTION(incmdSelectUnderCursoer, on_idle));
    }
    ...
    void on_idle(const EvType &ev) {
        std::string msg(
            "(idle) Selected " +
            QString::number(m_se.getObjects().size()).toStdString() +
            " shapes.");
        ...
        // else
        if (ev == MM)
            m_se.highlight_shape_under_pos(
                InteractiveCommandBase::get_last_point());
        else if (ev == MD)
            on_press(
                OTHER); // set_next_handler(HANDLE_FUNCTION(incmdSelectUnderCursoer,on_press));
        else
            return;
    }
  ...
  private:
    ...
    void on_press(const EvType &) {
        ...
        if (!m_se.getObjects().empty()) {
            ...
            set_next_handler(
                HANDLE_FUNCTION(incmdSelectUnderCursoer, on_mousemove));
        }
        // m_shape_added = true;
        //}
    }
...
class dicmdSelectShapesByRegion : public DirectCommandBase {
  ...
  public:
    ...
    virtual void execute() {
        ...
        m_reg = std::make_pair<QPoint, QPoint>(
            GET_CMD_ARG(PointCommandOptionValue, "-start"),
            GET_CMD_ARG(PointCommandOptionValue, "-end"));
        Selection::getInstance().clear();
        Selection::getInstance().find_and_highlightselect_shapes_from_region(
            m_reg);
        int count = Selection::getInstance().getObjects().size();
        std::string msg("Selected " + QString::number(count).toStdString() +
                        " shapes.");
        StatusBarManager::getInstance().updateStatusBar(msg.c_str(), 1, 0);
    ...
    virtual std::string get_name() { return "dicmdSelectShapesByRegion"; }
};
...
class dicmdSelectAllShapes : public DirectCommandBase {
  ...
  public:
    ...
    void execute() override { Selection::getInstance().highlightselect_all(); }

    std::string get_name() override { return "dicmdSelectAllShapes"; }
};

class incmdSelectShapesByRegion : public incmdCreateObj<RECTANGLE> {

  public:
    incmdSelectShapesByRegion(RuntimePoolManagerPtr r, ObjectPoolPtr s)
        : incmdCreateObj<RECTANGLE>(r, s) {
        m_first_click = true;
    }
    ...
    virtual void execute() {
        StatusBarManager::getInstance().updateStatusBar(
            "Click to select by region", 1, 0);

        ObjCreatorCommandBase<RECTANGLE>::set_next_handler(
            HANDLE_FUNCTION(incmdSelectShapesByRegion, on_idle));
...
public:
        incmdSelectShapesByRegion(ObjectPoolSandboxPtr r, IObjectPoolPtr s
):ObjCreatorCommandBase<RECTANGLE>(r,s)
    {
    }
