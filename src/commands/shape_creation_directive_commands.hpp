...
template <ObjectType T> class dicmdCreateObj : public DirectCommandBase {
  ...
  public:
    dicmdCreateObj(ObjectPoolPtr s)
        : ws(s) { // rq(RegionQuery::getInstance()) {
    ...
    dicmdCreateObj(const std::vector<PointCommandOptionValue> &pl,
                   const ShapeProperties &pr, ObjectPoolPtr s)
        : ws(s) {
        // m_pr = pr;
        ...
        add_option("-color", new StringCommandOptionValue(
                                 pr.brush_color.name().toStdString()));
        add_option("-brush",
                   new IntCommandOptionValue(pr.toStringsMap()["brush"]));
        add_option("-fill",
                   new IntCommandOptionValue(pr.toStringsMap()["fill"]));
    }
    ...
    void dump(const std::string &f) {
        ...
        //(*cmd_stream) <<
        //(GET_CMD_ARG(PointListCommandOptionValue,"-points").to_string()+"\n").c_str();
    ...
    virtual void execute() {
        //* //std::vector<QPoint>
        // v(GET_CMD_ARG(PointListCommandOptionValue,"-points"));
