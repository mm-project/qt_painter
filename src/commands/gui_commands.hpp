...
class dicmdguiSelectComboValue : public NonTransactionalDirectCommandBase
{
  ...
  public:
    ...
    dicmdguiSelectComboValue(
        const std::string &on,
        const std::string &ov) //: NonTransactionalDirectCommandBase("-object",new StringCommandOptionValue(on))
    {
        // FIXME add_option()
        m_on = on;
        m_ov = ov;
...
class dicmdguiClickModalButton : public NonTransactionalDirectCommandBase
{
  ...
  public:
    ...
    dicmdguiClickModalButton(const std::string &on)
        : NonTransactionalDirectCommandBase("-btn", new StringCommandOptionValue(on))
    {
        // FIXME add_option()
        // m_on = on;
    }
...
class dicmdguiClickButton : public NonTransactionalDirectCommandBase
{
  ...
  public:
    ...
    dicmdguiClickButton(const std::string &on)
        : NonTransactionalDirectCommandBase("-object", new StringCommandOptionValue(on))
    {
        // FIXME add_option()
        m_on = on;
    }
...
class dicmdCanvasMouseMove : public NonTransactionalDirectCommandBase
{
  ...
  public:
    ...
    {
        add_option("-point", new PointCommandOptionValue(QPoint(8, 8)));
...
class dicmdCanvasMousePress : public NonTransactionalDirectCommandBase
{
    ...
    QPoint m_p;
	bool m_need_mousemove = true;

  public:
    dicmdCanvasMousePress()
    {
        add_option("-point", new PointCommandOptionValue(QPoint(9, 9)));
    }

    dicmdCanvasMousePress(const QPoint &p, bool need_mousemove = true) : NonTransactionalDirectCommandBase("-point", new PointCommandOptionValue(p))
    {
        //assert(0);
		m_p = p;
		m_need_mousemove = need_mousemove;
...
class dicmdCanvasMouseRelease : public NonTransactionalDirectCommandBase
{
  ...
  public:
    ...
    dicmdCanvasMouseRelease(const QPoint &p, bool need_mousemove = true)
        : NonTransactionalDirectCommandBase("-point", new PointCommandOptionValue(p))
    {
        m_p = p;
		m_need_mousemove = need_mousemove;
    }
...
class dicmdCanvasMouseClick : public NonTransactionalDirectCommandBase
{
  ...
  public:
    ...
    virtual void execute()
    {
		...
		QApplication::processEvents();
        //dicmdCanvasMouseMove(m_p).execute();
        dicmdCanvasMouseRelease(m_p,false).execute();
		QApplication::processEvents();
		/**/
