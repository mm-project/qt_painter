...
public:
  ...
  dicmdguiSelectComboValue(
      const std::string &on,
      const std::string &ov) //: NonTransactionalDirectCommandBase("-object",new
                             //: StringCommandOptionValue(on))
  {
    // FIXME add_option()
    m_on = on;
    m_ov = ov;
  ...
  virtual std::string get_name() { return "dicmdguiSelectComboValue"; }
  ...
  virtual void execute() {
    ...
    std::cout << cmb->currentText().toStdString() << " " << m_on << " " <<
    cmb->findData("White") << std::endl;
...
public:
  ...
  dicmdguiClickModalButton(const std::string &on)
      : NonTransactionalDirectCommandBase("-btn",
                                          new StringCommandOptionValue(on)) {
    // FIXME add_option()
    // m_on = on;
  }

  virtual std::string get_name() { return "dicmdguiClickModalButton"; }
  ...
  virtual void execute() {
    ...
    // std::cout <<  it->objectName().toStdString() << "  " <<
    // it->text().toStdString() << std::endl; QAbstractButton* btn =
    // box->findChild<QAbstractButton*>(m_op1.c_str()); btn->click();
  }
};
...
public:
  ...
  dicmdguiClickButton(const std::string &on)
      : NonTransactionalDirectCommandBase("-object",
                                          new StringCommandOptionValue(on)) {
    // FIXME add_option()
    m_on = on;
  }

  virtual std::string get_name() { return "dicmdguiClickButton"; }
  ...
  virtual void execute() {
    ...
    m_on = GET_CMD_ARG(StringCommandOptionValue, "-object");
    // m_on =
    // (dynamic_cast<StringCommandOptionValue*>(get_option_val("-object")))->to_string();
...
public:
  ...
  dicmdguiSelectRadioButton(const std::string &on)
      : NonTransactionalDirectCommandBase("-object",
                                          new StringCommandOptionValue(on)) {
    // FIXME add_option()
    m_on = on;
  ...
  virtual std::string get_name() { return "dicmdguiSelectRadioButton"; }
...
public:
  ...
  dicmdCanvasMouseMove(const QPoint &p)
      : NonTransactionalDirectCommandBase("-point",
                                          new PointCommandOptionValue(p)) {
    m_p = p;
  }

  virtual std::string get_name() { return "dicmdCanvasMouseMove"; }

  // FIXME check dynamic_cast before *
  virtual void execute() {
    m_p = GET_CMD_ARG(PointCommandOptionValue, "-point");
    // m_p =
    // (dynamic_cast<PointCommandOptionValue*>(get_option_val("-point")))->get();
    QMouseEvent event(QEvent::MouseMove, m_p, Qt::LeftButton, Qt::NoButton,
                      Qt::NoModifier);
...
public:
  ...
  dicmdCanvasMouseClick(const QPoint &p)
      : NonTransactionalDirectCommandBase("-point",
                                          new PointCommandOptionValue(p)) {
    m_p = p;
  }

  virtual std::string get_name() { return "dicmdCanvasMouseClick"; }
  ...
  virtual void execute() {
    ...
    QMouseEvent event(QEvent::MouseButtonPress, m_p, Qt::LeftButton,
...
public:
  ...
  dicmdCanvasMousePress(const QPoint &p)
      : NonTransactionalDirectCommandBase("-point",
                                          new PointCommandOptionValue(p)) {
    m_p = p;
  }

  virtual std::string get_name() { return "dicmdCanvasMousePress"; }
  ...
  virtual void execute() {
    ...
    m_p = GET_CMD_ARG(PointCommandOptionValue, "-point");
    dicmdCanvasMouseMove(m_p).execute();
    QMouseEvent event(QEvent::User, m_p, Qt::LeftButton, Qt::NoButton,
                      Qt::NoModifier);
    QApplication::sendEvent(CM->findChild<QWidget *>("CANVAS"), &event);
  }
};
...
public:
  dicmdCanvasMouseRelease() {
    add_option("-point", new PointCommandOptionValue(QPoint(0, 0)));
  }

  dicmdCanvasMouseRelease(const QPoint &p)
      : NonTransactionalDirectCommandBase("-point",
                                          new PointCommandOptionValue(p)) {
    m_p = p;
  }

  virtual std::string get_name() { return "dicmdCanvasMouseRelease"; }
  ...
  virtual void execute() {
    ...
    m_p = GET_CMD_ARG(PointCommandOptionValue, "-point");
    dicmdCanvasMouseMove(m_p).execute();
    QMouseEvent event(QEvent::MouseButtonRelease, m_p, Qt::LeftButton,
                      Qt::NoButton, Qt::NoModifier);
    QApplication::sendEvent(CM->findChild<QWidget *>("CANVAS"), &event);
  }
};
...
public:
  ...
  dicmdCanvasMouseDblClick(const QPoint &p)
      : NonTransactionalDirectCommandBase("-point",
                                          new PointCommandOptionValue(p)) {
    m_p = p;
  }

  virtual std::string get_name() { return "dicmdCanvasMouseDblClick"; }
  ...
  virtual void execute() {
    ...
    m_p = GET_CMD_ARG(PointCommandOptionValue, "-point");
    dicmdCanvasMouseMove(m_p).execute();
    QMouseEvent event(QEvent::MouseButtonDblClick, m_p, Qt::LeftButton,
                      Qt::NoButton, Qt::NoModifier);
    QApplication::sendEvent(CM->findChild<QWidget *>("CANVAS"), &event);
  }
};
