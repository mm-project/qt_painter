...
private:
  void on_commit(const EvType &) {
    if (T == LOAD) {
      if (is_agreed_with_user()) {
        m_fn = QFileDialog::getOpenFileName(0, "Load Design", "", "*.*", 0,
      ...
      if (is_agreed_with_user()) {
        m_fn = QFileDialog::getSaveFileName(0, "Save Design", "", "*.*", 0,
                                            QFileDialog::DontUseNativeDialog)
                   .toStdString();
  ...
  bool is_agreed_with_user() {
    // if ( ! is_saved() )
    // return mmModalDialog::ask_yn_question("Design"+m_helpstr,"Your design has
    // not been saved yet. Do you want to continue with "+m_helpstr+"?");
    return true;
  }
};
