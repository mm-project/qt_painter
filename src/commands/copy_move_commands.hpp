...
template <relocAction T> class dicmdObjRelocateBy : public DirectCommandBase {
...
class incmdObjRelocateBy : public InteractiveCommandBase {
  ...
  public:
    incmdObjRelocateBy(RuntimePoolManagerPtr r, ObjectPoolPtr s)
        : m_re(r), m_ws(s) {
    ...
    virtual void handle_update() {
        if (m_move_move)
            for (auto it : m_sb->getObjects()) {
                it->updateProperties(
                    controller::getInstance().get_shape_properties());
  ...
  private:
    ...
    void idle(const EvType &) {
        // std::cout << "COPYMOVE IDLE" <<  m_se.getObjects().size() <<
        // std::endl;
        //  no selection, invoke selectbyregion to select object firsts and
        //  return
        if (m_se.getObjects().empty()) {
            // abort();
            m_cm.return_to_idle();
            return;
            LeCallback cb = REGISTER_CALLBACK(
                OBJECT_SELECTED, &incmdObjRelocateBy<T>::on_object_selected);
            if (m_sel_cb == nullptr)
        ...
        std::string msg(
            "Selected " + QString::number(count).toStdString() +
            " shapes. Click on shape and move mouse to perform action");
        StatusBarManager::getInstance().updateStatusBar(msg.c_str(), 1, 0);
        // create runtime objects from selected objects so we can move them with
        // cursor keep mapping runtimeobj<->realobj std::cout << "XXX SELECTION"
        // << m_se.getObjects().size() << "   RTSHAPES: " <<
        // m_sb->getPool()->getObjects().size() << "\n";
        for (auto it : m_se.getObjects()) {
            IShapePtr i = m_sb->addObject(it);
            m_sb2se[i] = it;
        }
        // std::cout << "YYY SELECTION" << m_se.getObjects().size() << "
        // RTSHAPES: " << m_sb->getPool()->getObjects().size() << "\n";

        // wait for user to start the movecopy by clicking.
        set_next_handler(
            HANDLE_FUNCTION(incmdObjRelocateBy<T>, wait_for_first_click));
    }
    ...
    void wait_for_first_click(const EvType &ev) {
        if (ev == MC || ev == MU || ev == MD) {
            ...
            for (auto shape : m_se.getObjects()) {
                ...
                // std::cout << m_clicked_point.x() << " " <<
                // m_clicked_point.y() << " shape " << shape << " " <<
                // shape->getPoints()[0].x() << " " << shape->getPoints()[0].y()
                // << " diff "<< diff.x() << " " << diff.y()
                // << std::endl;
            ...
            m_move_move = true;
            set_next_handler(
                HANDLE_FUNCTION(incmdObjRelocateBy<T>, act_on_mousemove));
            StatusBarManager::getInstance().updateStatusBar(
                "Click on desired place to commit action", 1, 0);
        }
    }
