...
template <relocAction T> class incmdObjRelocateBy : public InteractiveCommandBase
{
  ...
  private:
    ...
    void idle(const EvType &)
    {
        ...
        // std::cout << "XXX SELECTION" << m_se.getObjects().size() << "   RTSHAPES: " <<
        // m_sb->getPool()->getObjects().size() << "\n";
        for (auto it : m_se.getObjects())
        {
    ...
    void wait_for_first_click(const EvType &ev)
    {
        ...
        if (ev == MC)			
        {
            ...
            for (auto shape : m_se.getObjects())
            {
                ...
                // std::cout << m_clicked_point.x() << " " << m_clicked_point.y() << "  shape " << shape << " " <<
                // shape->getPoints()[0].x() << " " << shape->getPoints()[0].y()  << " diff "<< diff.x() << " " <<
                // diff.y() << std::endl;
