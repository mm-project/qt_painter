...
private:
  struct Node {
    ...
    std::vector<T> items; // shapes stored here when they straddle boundaries or
                          // node is leaf
  ...
  void queryRangeNode(const Node *n, const Box &range, F &&cb) const {
    if (!n->box.intersects(range))
      return;
    for (const auto &it : n->items) {
      // For IShapePtr, use the shape's intersects method for more accurate
      // testing
...
public:
  ...
  explicit QtShapeQuadtree(Box world = Box{-1000000.0, -1000000.0, 1000000.0,
                                           1000000.0},
                           std::size_t capacity = 16, std::size_t maxDepth = 32,
