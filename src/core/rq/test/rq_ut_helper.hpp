...
template <typename T>
void create_obj_at_given_cell_and_row(int column, int row, bool ws) {
  // don't know, but it does what i wanted
  int delta = 100;
  int epsilon = 90;
  create_obj<T>(delta * row, delta * column, delta * row - epsilon,
                delta * column - epsilon, ws);
}
...
template <typename T> void insert_nxn_matrix_of_objs(int n) {
  std::cout << "  inserting " << n << "x" << n << " objects to ws ..."
            << std::endl;
  // the_ws.reserve(n*n);
  ...
  ws_insert_ms = std::chrono::duration_cast<std::chrono::microseconds>(
                     ws_insert_end - ws_insert_start)
                     .count();
  std::cout << "       --> ws insert took " << ws_insert_ms << std::endl;

  std::cout << "  inserting " << n << "x" << n << " objects to rq ..."
            << std::endl;
  ...
  rq_insert_ms = std::chrono::duration_cast<std::chrono::microseconds>(
                     rq_insert_end - rq_insert_start)
                     .count();
...
void validate_rq(int x, int y, int width, int height) {
  std::cout << "  [validation " << validation_id << "]: proceeding query ..."
            << std::endl;

  ///*
  auto it = skip_lists.find(test_id);
  if (it != skip_lists.end() &&
      it->second.find(validation_id) != it->second.end()) {
  ...
  if (allow_lists.size() > 0 &&
      (it2 == allow_lists.end() ||
       it2->second.find(validation_id) == it2->second.end())) {
    std::cout << "       skipped" << std::endl;
    refresh_drawbles();
    validation_id++;
  ...
  auto rq_duration =
      std::chrono::duration_cast<std::chrono::microseconds>(rq_end - rq_start)
          .count();
  std::cout << "       --> rq lookup took " << rq_duration << "ms and got "
            << rq_shapes.size() << " objects" << std::endl;
  ...
  auto ws_duration =
      std::chrono::duration_cast<std::chrono::microseconds>(ws_end - ws_start)
          .count();
  std::cout << "       --> ws lookup took " << ws_duration << "ms and got "
            << ws_shapes.size() << " objects" << std::endl;
  ws_query_ms = ws_duration;

  if (rq_shapes.size() != ws_shapes.size()) {
    std::cout << "       Issue found!" << std::endl;
    failed_tests.push_back(test_info(test_id, validation_id, rq_shapes.size(),
                                     ws_shapes.size(),
  ...
  if (need_drawing) {
    ...
    for (auto &shape : the_ws)
      shape->draw(&painter);
    auto select_rect =
        create<Rectangle>(QPoint(x, y), QPoint(x + width, y + height));
    ShapeProperties p;
    p.pen_color = Qt::red;
    ...
    QString fname_des = "des_" + QString::number(test_id) + "_" +
                        QString::number(validation_id) + ".png";
    ...
    QString fname_rq = "rq_" + QString::number(test_id) + "_" +
                       QString::number(validation_id) + ".png";
    std::cout << "        saving rq data to " << fname_rq.toStdString()
              << " ..." << std::endl;
    ...
    QString fname_ws = "ws_" + QString::number(test_id) + "_" +
                       QString::number(validation_id) + ".png";
    std::cout << "        saving ws data to " << fname_ws.toStdString()
              << " ..." << std::endl;
...
void generate_html_diff() {
  ...
  for (auto &failed_test : failed_tests) {
    ...
    const std::string des =
        "des_" + std::to_string(t_id) + "_" + std::to_string(v_id) + ".png";
    const std::string rq =
        "rq_" + std::to_string(t_id) + "_" + std::to_string(v_id) + ".png";
    const std::string ws =
        "ws_" + std::to_string(t_id) + "_" + std::to_string(v_id) + ".png";

    html += "<tr>\n";
    html += "  <td>Test " + std::to_string(t_id) + " &mdash; Validation " +
            std::to_string(v_id) + "</td>\n";

    // Each image calls showImage('path') to open overlay
    html += "  <td><img src=\"" + des +
            "\" alt=\"design\" onclick=\"showImage('" + des + "')\"></td>\n";

    html += "  <td><img src=\"" + rq + "\" alt=\"rq\" onclick=\"showImage('" +
            rq +
            "')\">"
            "<br>RQ Got " +
            std::to_string(rq_oc) + " objs</td>\n";

    html += "  <td><img src=\"" + ws + "\" alt=\"ws\" onclick=\"showImage('" +
            ws +
            "')\">"
            "<br>WS Got " +
...
void print_results() {
  ...
  if (failed_tests.size() != 0) {
    for (auto &failed_test : failed_tests) {
      std::cout << "Failed: test" << failed_test.test_id << " in validation "
                << failed_test.validation_id << std::endl;
    }
    std::cout << std::endl;
...
void skip_test(int t_id, int v_id) { skip_lists[t_id].insert(v_id); }
...
void write_results_graph_html(const std::map<int, std::vector<int>> &results) {
  ...
  if (x_min == std::numeric_limits<int>::max()) {
    x_min = 0;
    x_max = 1;
  }
  auto safeTop = [](int v) {
    return (v == std::numeric_limits<int>::min()) ? 1 : v;
  };
...
<body>
  ...
  <script>
    ...
    const yInsBothMax = )"
      << safeTop(y_ins_both_top) << R"( * 1.05;
    const yQryBothMax = )"
      << safeTop(y_qry_both_top) << R"( * 1.05;
    const yInsWsMax   = )"
      << safeTop(y_ins_ws_top) << R"( * 1.05;
    const yInsRqMax   = )"
      << safeTop(y_ins_rq_top) << R"( * 1.05;
    const yQryWsMax   = )"
      << safeTop(y_qry_ws_top) << R"( * 1.05;
    const yQryRqMax   = )"
      << safeTop(y_qry_rq_top) << R"( * 1.05;
...
void do_rq_perf_test(int repeat_factor, std::vector<int> &magnitudes,
  ...
  std::cout << std::endl;
  std::cout << "magnitude, ws_insert, rq_insert, ws_query, rq_query"
            << std::endl;
  for (auto &[key, data] : results) {
    std::cout << key << "," << data[0] << "," << data[1] << "," << data[2]
              << "," << data[3] << std::endl;
