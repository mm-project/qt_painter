...
void generate_html_diff() {
    ...
    for (auto &failed_test : failed_tests) {
        ...
        html += "  <td><img src=\"" + des +
                "\" alt=\"design\" onclick=\"showImage('" + des +
                "')\"></td>\n";

        html += "  <td><img src=\"" + rq +
                "\" alt=\"rq\" onclick=\"showImage('" + rq +
                "')\">"
                "<br>RQ Got " +
                std::to_string(rq_oc) + " objs</td>\n";

        html += "  <td><img src=\"" + ws +
                "\" alt=\"ws\" onclick=\"showImage('" + ws +
                "')\">"
                "<br>WS Got " +
...
void print_results() {
    ...
    if (failed_tests.size() != 0) {
        for (auto &failed_test : failed_tests) {
            std::cout << "Failed: test" << failed_test.test_id
                      << " in validation " << failed_test.validation_id
                      << std::endl;
        }
        std::cout << std::endl;
