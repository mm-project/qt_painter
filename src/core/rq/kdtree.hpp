...
template <typename T> using KDnodePtr = std::shared_ptr<KDnode<T>>;

template <typename T> class KDtree;
template <typename T> using KDtreePtr = std::shared_ptr<KDtree<T>>;
...
template <typename T> class KDtree : public IRQtree<T> {
public:
  KDtree() = default;
...
template <typename T> KDtree<T>::KDtree(const std::vector<T> &arrObjecrs) {
  insert(arrObjecrs);
}

template <typename T> void KDtree<T>::initRoot() {
  if (m_pRoot != nullptr)
    return;
...
template <typename T> void KDtree<T>::insert(const std::vector<T> &arrObjecrs) {
  initRoot();
...
template <typename T> void KDtree<T>::insert(const T &object) {
  initRoot();
...
template <typename T> void KDtree<T>::clear() { m_pRoot = nullptr; }
...
template <typename T> void KDtree<T>::remove(const T &object) {
  // TODO: make it better
