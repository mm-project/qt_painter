...
std::string Selection::getName() const noexcept { return "Selection"; }
...
void Selection::temporary_highlight() { m_sel_highlight_set->highlight_on(); }
 ...
 *   and add them to local set ( vector ).
 ! */
void Selection::find_and_highlightselect_shapes_from_region(
...
void Selection::highlight_last_selected_region(bool on_off) {
  ...
  std::cout << m_last_region.bottomRight().x() << " "
            << m_last_region.bottomRight().y() << std::endl;
  // use shape creator instead of clone
  m_qa_highlight_set->addObject(std::shared_ptr<IShape>(
      Rectangle(m_last_region, ShapeProperties()).clone()));
  m_qa_highlight_set->highlight_on();
}

//*********************************** HIGHLIGHT
//********************************************
HighlightSet::HighlightSet(const std::string &n, const ShapeProperties &p)
    : m_name(n), m_packet(p) {}
...
std::string HighlightSet::getName() const noexcept { return m_name; }

void HighlightSet::highlight_on() { highlight_on_off(true); }
