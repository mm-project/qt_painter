...
LeCallback LePostman::register_callback(const std::string &n,
                                        const LeCallbackType &t,
                                        callBackFun1 f) {
    std::cout << "1will reg callback " << n << " with id"
              << m_type2vecfun[t].size() + 1 << std::endl;

    m_type2vecfun[t].push_back(
        LeCallback(n, f, m_type2vecfun[t].size() + 1, t));
...
LeCallback LePostman::register_callback(const std::string &n,
                                        const LeCallbackType &t,
                                        callBackFun2 f) {
    std::cout << "2will reg callback " << n << " with id"
              << m_type2vecfun[t].size() + 1 << std::endl;
...
void LePostman::deregister_callback(const std::string &n, LeCallbackType &t,
                                    int id) {
...
// void LePostman::notify(const std::string& n, const LeCallbackType&
// t,LeCallbackData data) {
//         notify(n,t,data)
// }
...
void LePostman::notify(const std::string &, const LeCallbackType &t,
    ...
    for (auto it : m_type2vecfun[t]) {
        // std::cout << n << " function is calling " << it.get_name() << " via
        // ENUM:" << t << std::endl; std::cout << n << " called to " << " via "
        // << std::endl;
        it.call(data);
    }
}
