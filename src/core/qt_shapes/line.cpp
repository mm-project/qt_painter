...
// @Line implementation
//
Line::Line(QLineF l, ShapeProperties p)
    : IShape(LINE, p), m_waitForSecondClick(false) {
    m_object = l;
}

Line *Line::clone() { return new Line(m_object, m_properties); }
...
void Line::setP1(const QPoint &p) { m_object.setP1(p); }

void Line::setP2(const QPoint &p) { m_object.setP2(p); }

QPoint Line::getP1() const { return m_object.toLine().p1(); }

QPoint Line::getP2() const { return m_object.toLine().p2(); }
...
bool Line::contains(const QPoint &p) const {
    ...
    const float EPSILON =
        20; // this is supposed to be small but we don't use floating system
...
QPoint Line::center() const { return m_object.toLine().center(); }

bool Line::isDisjointFrom(const QRect &) const { return false; }

QRectF Line::getBBox() const { return QRectF(m_object.p1(), m_object.p2()); }