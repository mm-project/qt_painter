...
Rectangle::Rectangle(QRect r, ShapeProperties p)
    : IShape(RECTANGLE, p), m_waitForSecondClick(false) {
  m_object = r;
}

Rectangle *Rectangle::clone() { return new Rectangle(m_object, m_properties); }
...
void Rectangle::setTopLeft(const QPoint &p) { m_object.setTopLeft(p); }

void Rectangle::setBottomRight(const QPoint &p) { m_object.setBottomRight(p); }

QPoint Rectangle::getTopLeft() const { return m_object.topLeft(); }

QPoint Rectangle::getBottomRight() const { return m_object.bottomRight(); }
...
QPoint Rectangle::center() const { return m_object.center(); }

bool Rectangle::isDisjointFrom(const QRect &oRect) const {
  return !oRect.contains(m_object);
}

QRectF Rectangle::getBBox() const { return QRectF(m_object); }
