...
Polygon *Polygon::clone() { return new Polygon(m_object, m_properties); }
...
QPoint Polygon::getTopLeft() const { return m_object.boundingRect().topLeft(); }
...
bool Polygon::isDisjointFrom(const QRect &) const { return false; }

bool Polygon::contains(const QPoint &point) const {
  return m_object.containsPoint(point, Qt::OddEvenFill);
}

QRectF Polygon::getBBox() const { return QRectF(m_object.boundingRect()); }
