...
#define REGISTER_CALLBACK(A, B)                                                                                        \
    LePostman::getInstance().register_callback(std::string(STR(A)) + std::string("-->") + std::string(STR(B)), A,      \
...
#define NOTIFY(A, B)                                                                                                   \
    LePostman::getInstance().notify(std::string(typeid(this).name()) + std::string(__FUNCTION__), A, B);
