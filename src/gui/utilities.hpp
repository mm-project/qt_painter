...
bool is_w_from_desired_list(QWidget *w) {
    return (dynamic_cast<QAbstractButton *>(w) ||
            dynamic_cast<QTabWidget *>(w) ||
            dynamic_cast<QTabBar *>(w)); // || qobject_cast<QAbsractButton*>(w)
}
/*
void process_unnamed(QWidget* w) {
    std::cout << "Error: Found widget with no texting property or objectName.
Devs please do something. Widget's Hier path : " << cn.toStdString() << " in "
...
void set_object_name_for_logging(QWidget *w, bool needparentinfo = true) {
    ...
    // by traversing up to the most top parent, and concetating all parents
    // objectnames
    QString pn = "";
    if (needparentinfo) {
        QObject *p = w->parent();
    ...
    // try to get some other info as it's name
        ...
        // if nothing worked,
        // print warning, and assign name as type + i , like QPushButon1,
        // QToolButton2...
        if (wn.isEmpty()) {
            if (isleaf && is_w_from_desired_list(w)) {
                // fixme : if only in test-mode !
                std::cout << "Error: Found widget with no texting property or "
                             "objectName. Devs please do something. "
                             "Widget's Hier path : "
                          << cn.toStdString() << " in " << pn.toStdString()
                          << "\n";
                // Messenger::expose_msg(warn,"No locator for
                // "+cn.toStdString()+" in " +pn.toStdString());
    ...
    if (isleaf && is_w_from_desired_list(w))
        w->setObjectName(get_hashed_name(pn) +
                         wn); // want a parent name squized-hashed
    else
