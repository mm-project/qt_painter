...
bool mmModalDialog::ask_yn_question(const std::string &tl,
                                    const std::string &bd) {
  ...
  if (Application::is_replay_mode()) {
    ...
    QMessageBox::StandardButton reply =
        QMessageBox::question(m_parent_w, tl.c_str(), bd.c_str());
...
bool mmModalDialog::critical(const std::string &tl, const std::string &bd) {
  ...
  if (Application::is_replay_mode()) {
    ...
    QMessageBox::StandardButton reply =
        QMessageBox::critical(m_parent_w, tl.c_str(), bd.c_str());
