...
QRibbonGroup::QRibbonGroup(QWidget *p) : QWidget(p), m_row(0), m_column(0) {
...
void QRibbonGroup::setTitle(const QString &title) { m_label->setText(title); }

QString QRibbonGroup::title() const { return m_label->text(); }
...
void QRibbonGroup::addButton(QWidget *button, const QString &label,
                             QRibbonButtonSize size) {
    if (size == QRibbonButtonSize::size46 ||
        size == QRibbonButtonSize::size32) {
        QVBoxLayout *l = new QVBoxLayout;
        (size == QRibbonButtonSize::size32) ? button->setFixedSize(32, 32)
...
void QRibbonGroup::addRibbonButton(QRibbonButton *button,
                                   QRibbonButtonSize buttonSize) {
    switch (buttonSize) {
