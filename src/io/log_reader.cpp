...
void LogReader::reply_stop(LeCallbackData &) {
  ...
  Application::getInstance().set_replay_mode(false);
  Messenger::expose_msg(info, "Replying Stopped for debug. Press \"W\" to "
                              "continue to run till next breakpoint, or "
                              "\"S\" to execute next command.");
}

bool LogReader::is_paused() { return m_paused; }
...
QStringList LogReader::read_file(const std::string &fname) {
  ...
  if (!file.exists() || !file.open(QIODevice::ReadOnly)) {
    // fixme call mmModalDialog , that will also put error message to console
    // and log
    Application::getInstance().set_replay_mode(false);
    // mmModalDialog::critical("Log replay error", "Can't open file "+fname+" to
    // replay");
    return stringList;
  }
...
bool LogReader::replay_logfile(const std::string &fname) {
  ...
  if (!QString::fromLocal8Bit(qgetenv("ELEN_PAINTER_STARTDBG").constData())
           .isEmpty())
...
void LogReader::execute_next_command() {
  ...
  CommandBase *cmd =
      m_interp.get_cmd_obj(m_command_queue.front().toStdString());
  m_command_queue.pop();
